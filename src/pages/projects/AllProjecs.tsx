...
import "./community-service/CommunityService.css";
      ...
      <div className="projects-list">
        {allProjects.map((project, index) => (
          <div key={index} className="project-card">
            ...
            <p className="project-text">{project[0].description}</p>

            {/* Impact Metrics */}
            <div className="impact-metrics">
              <p>Impact: {project[0].impact}</p>
            </div>

            {/* Testimonial */}
            <div className="testimonial">
              <p>"{project[0].testimonial}"</p>
            </div>
            ...
            <div className="gallery">
              ...
              <div className="gallery-images">
                {project[0].gallery.map((image, idx) => (
                  <img key={idx} src={image} alt={`${project[0].title} ${idx + 1}`} className="gallery-image" />
                ))}
              </div>
            </div>

            {/* Call-to-Detail Button */}
            <Link to={`/community-service/projectDetail/${project[0].id}`} className="cta-button">
              Learn More
            </Link>
          </div>
        ))}
