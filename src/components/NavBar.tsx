    ...
    <div className={`navbar-container ${showNav ? 'show' : 'hidden'}`}>
      <div className="navbar-main">
        ...
        <div className="navbar-links">
          ...
          >
            <span className="nav-link projects-toggle">
              {t('projectss')}
            </span>
          ...
          <Link to="/directorship-positions" className="nav-link">{t('directorship_position')}</Link>
          <Link to="/contact-us" className="nav-link">{t('contact_us')}</Link>
          <Link to="/about" className="nav-link">{t('about_us')}</Link>
          <select className='language-selector' value={i18n.language} onChange={(e) => i18n.changeLanguage(e.target.value)}>
            <option value="en">EN</option>
            <option value="fr">FR</option>
          </select>
          <Link to="/donate" className="submit-button nav-link">{t('donate')}</Link>
        </div>
      </div>
    </div>
  );
